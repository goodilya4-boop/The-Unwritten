# Roadmap

## Phase 0 — Project Foundation
- [x] Создать NeoForge 1.21.1 проект
- [x] Настроить Java 21
- [x] Настроить Mod ID
- [x] Настроить package
- [x] Проверить запуск Minecraft
- [ ] Настроить Git
- [x] Создать базовую документацию
- [ ] Очистить MDK и локальные артефакты

## Phase 1 — Character Foundation
- [x] Атрибуты
- [x] Производные характеристики
- [x] Ресурсы
- [x] Player Data
- [x] Сохранение
- [ ] Интерфейс персонажа

## Phase 1.5 — Modifier & Equipment Foundation
- [x] Единая модель AttributeModifier
- [x] ADDITION / MULTIPLY_BASE / MULTIPLY_TOTAL
- [x] Удалить legacy ItemAttributeModifier/ItemAttributeHelper
- [x] Перевести CharacterStats на эффективные атрибуты
- [x] Уникальность модификатора по (source, id)
- [x] Жизненный цикл модификаторов экипировки
- [x] Базовая интеграция предметных компонентов с конкретным предметом
- [x] Базовый предмет экипировки для проверки lifecycle
- [x] Unit-тесты reconciliation: equip / replace / unequip / repeat

## Phase 2 — Progression Foundation
- [ ] Знания
- [ ] Предрасположенность
- [ ] Специализация
- [ ] Требования
- [ ] Мастерство
- [ ] Двойные пути

## Phase 3 — Combat Foundation
- [ ] Базовый бой
- [ ] Техники владения
- [ ] Воинские школы
- [ ] Анимации

## Phase 4 — Magic Foundation
- [ ] Магический ресурс
- [ ] Школы магии
- [ ] Заклинания
- [ ] Магические организации
- [ ] Двойные пути

## Phase 5 — Engineering Foundation
- [ ] Ресурсы
- [ ] Инструменты
- [ ] Устройства
- [ ] Инженерные школы

## Phase 6 — Hybrid Systems
- [ ] Взаимодействия путей
- [ ] Возникновение новых школ
- [ ] Гибридные техники
- [ ] Гибридные заклинания
- [ ] Гибридные инженерные системы

## Phase 7 — World
- [ ] Мобы
- [ ] NPC
- [ ] Фракции
- [ ] Структуры
- [ ] События

## Phase 8 — Story
- [ ] Пустота
- [ ] Создание мира
- [ ] Вступление
- [ ] Диалоги
- [ ] Выборы
- [ ] Последствия

## Phase 9 — Polish
- [ ] UI
- [ ] Анимации
- [ ] Частицы
- [ ] Звуки
- [ ] Оптимизация
